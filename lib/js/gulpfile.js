...
gulp.task('js2', watchify((watchify) => {
  gulp
    .src(jsWatch)
    .pipe(plumber())
    .pipe(watch(jsWatch))
...
gulp.task('js', () => {
  let base = process.cwd() + '/src/'

  glob(jsWatch, (_, files) => {
    files.forEach((f) => {
      gulp
        .src(f, { read: false })
        .pipe(shell(
          ['watchify -t babelify --extension=js <%= file.path %> -o <%= f(file.path) %> -v'],
          { templateData: { f: (s) => s.replace(base, '../../public/js/') } }
        ))
    })
  })
})
