...
class ItemList extends React.Component {
  ...
  get items () {
    ...
    return <ul className="report-item-list">
      {items.map(({ id, name, root }, index) => {
        return <li className="report-item">
          ...
          <div className="input"><input type="text" className="report-item-name" value={name} onChange={(e) => this.changeItem(index, e)}/></div>
          <div className="delete">{this.detectDeleteButton(id, root, index)}</div>
        </li>
      })}
    </ul>
  }

  detectDeleteButton (id, root, index) {
    if (root) {
      return <span>required.</span>
    }

    return id
      ? <span><input type="checkbox" value={id} onChange={this.toggleItem}/> delete on saving</span>
