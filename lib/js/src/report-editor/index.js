...
class ReportEditor extends React.Component {
  ...
  takeInState (rawParams) {
    let { id: reportId, reportConfigurationId, name, sections, timing } = new Report(rawParams)
    this.setState({ reportId, reportConfigurationId, name, sections, timing })
  }
  ...
  submit () {
    ...
    let { name, reportId, reportConfigurationId, sections } = this.state
    this.sendTargetAPI({ name, reportId, reportConfigurationId, sections })
      .then((params) => {
  ...
  updateReport () {
    ...
    let { reportId, reportConfigurationId, name, items } = this.state
    API.updateReport({ reportId, reportConfigurationId, name, items })
      .then((params) => {
        this.takeInState(params)
        this.setState({ condition: Condition.Waiting })
      })
...
class ReportSectionEditor extends React.Component {
  ...
  get content () {
    ...
    return this.state.isPreview
      ? <section class="preview" dangerouslySetInnerHTML={{ __html: marked(raw) }}/>
