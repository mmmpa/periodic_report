...
import reportConfigurationConfiguration from '../../lib//api-configuration/report-group'
import { bind } from 'decko'
import Fa from '../../lib//components/fa'
import { Condition } from '../../lib/models/condition'

registerAPI(reportConfigurationConfiguration)

window.ReportConfigurationEditorRunner = class {
  static run (dom, params) {
    ReactDOM.render(<ReportConfigurationEditor {...params}/>, dom)
  }
}

@receiver
@sender
class ReportConfigurationEditor extends React.Component {
  componentWillMount () {
  ...
  get sendTargetAPI () {
    return this.state.id
      ? API.updateReportConfiguration
      : API.createReportConfiguration
  }
  ...
  takeInState (rawParams) {
    let { id, name, report_section_configurations:items, timing } = rawParams
